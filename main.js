...
// ╔══════════════════════════════════════════════════════════╗
// ║  >>> UBAH DI SINI (BARIS 12) <<<                         ║
// ║                                                          ║
// ║  Mau pakai URL internet?  →  true                        ║
// ║  Mau pakai file lokal?    →  false                       ║
// ╚══════════════════════════════════════════════════════════╝
...
function transpile(source) {
  ...
  const readString = (quote) => {
    ...
    while (i < n) {
      const c = source[i];
      if (c === "\n") throw new JawaError("String durung rampung", sl, sc);
      s += c;
      if (c === "\\") {
        advance();
        if (i < n) {
  ...
  const readTemplate = () => {
    ...
    throw new JawaError("Template durung rampung", sl, sc);
  };
  ...
  while (i < n) {
    ...
    if (ch === "/" && next === "*") {
      ...
      if (end === -1) throw new JawaError("Komentar durung rampung", sl, sc);
      copy(end + 2 - i);
      continue;
    }
    if (ch === "/" && regexAllowed()) {
...
function describeError(err, source = "") {
  ...
  const hint = isErr && err.name === "SyntaxError"
    ? "\n\nTips: jalanke \"JawaScript: Tampilkan JavaScript hasil compile\" kanggo ndeleng kode JS-ne."
    : "";
...
function getSource() {
  ...
  if (!view) throw new Error("Ora ana editor sing aktif.");
...
function pushLine(run, text) {
  if (run.output.length >= MAX_OUTPUT_LINES) {
    if (!run.truncated) { run.truncated = true; run.output.push("… output dipotong (kakehan baris)"); }
    return;
  }
...
function renderRun(run) {
  ...
  if (run.status === "running") {
    kind = "running";
    if (!body) body = "Lagi mlaku…";
  } else if (run.status === "error") {
    kind = "error";
    body += (body ? "\n\n" : "") + describeError(run.error, run.source);
  } else if (!body) {
    body = "Program selesai tanpa output.";
  }
...
async function handleRun() {
  ...
  if (!/\.jawa$/i.test(fileName)) {
    const ok = await askConfirm(
      "Dudu file .jawa",
      `"${fileName}" dudu file .jawa. Kata kunci Jawa bisa ngrusak kode JavaScript biasa. Tetep dijalanke?`
    );
...
function init(baseUrl, $page) {
  ...
  commandsRef.addCommand({
    name: RUN_COMMAND,
    description: "JawaScript: Run file aktif",
  ...
  commandsRef.addCommand({
    ...
    description: "JawaScript: Tampilkan JavaScript hasil compile",
    exec: handleCompile
  });
  ...
  } catch (e) { console.warn("Gagal register bahasa:", e); }

  // 3. Ikon Tab — background-image biar ukurannya kekontrol
  try {
    const logoUrl = USE_REMOTE_ICON
      ? ICON_URL
      : `${baseUrl}/${ICON_LOCAL_FILE}`;

    console.log("[JawaScript] Mode ikon:", USE_REMOTE_ICON ? "URL" : "LOKAL");
    console.log("[JawaScript] URL ikon:", logoUrl);
  ...
  } catch (e) { console.warn("Gagal inject CSS ikon:", e); }
  ...
  } catch (e) { console.warn("SideButton tidak tersedia:", e); }
}
