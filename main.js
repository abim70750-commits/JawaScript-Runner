...
const EXPORT_COMMAND = "jawascript.export";
const REPO_VERSION = "1.4.0";
...
async function handleExport() {
  ...
  try {
    ...
    showPanel(pageRef, "Export Error", describeError(err), "error");
    return;
  }

  if (!/\.jawa$/i.test(fileName)) {
    showPanel(pageRef, "Export Failed", "Only .jawa files can be exported to JavaScript.", "error");
    return;
  }

  try {
    const js = transpile(source);
    const jsFileName = fileName.replace(/\.jawa$/i, ".js");

    const activeFile = window.editorManager?.activeFile;
    const fileUrl = activeFile?.uri || activeFile?.url || activeFile?.id || "";

    if (!fileUrl || !fileUrl.includes("/")) {
      showPanel(pageRef, "Export Failed", "Could not determine the file location. Please save the file first.", "error");
      return;
    }

    const dir = fileUrl.substring(0, fileUrl.lastIndexOf("/"));
    const targetUrl = `${dir}/${jsFileName}`;

    const fsOperation = acode.require("fsOperation");
    if (!fsOperation) {
      showPanel(pageRef, "Export Failed", "File system API is not available.", "error");
      return;
    }

    const file = fsOperation(targetUrl);
    if (!(await file.exists())) {
      await file.createFile("", "");
    }
    await file.writeFile(js);

    showPanel(pageRef, "Export Success", `Saved to: ${jsFileName}`, "success");
  } catch (err) {
    showPanel(pageRef, "Export Error", describeError(err, source), "error");
  }
}
...
function jawaCompletionSource(context) {
  const word = context.matchBefore(/[\p{L}\p{N}_$]*/u);
  if (!word) return null;
  if (word.from === word.to && !context.explicit) return null;

  const keywords = [
    { label: "yen", type: "keyword", detail: "if" },
    { label: "ora", type: "keyword", detail: "else" },
    { label: "kanggo", type: "keyword", detail: "for loop" },
    { label: "nganti", type: "keyword", detail: "while loop" },
    { label: "fungsi", type: "keyword", detail: "function" },
    { label: "kelas", type: "keyword", detail: "class" },
    { label: "balekno", type: "keyword", detail: "return" },
    { label: "buang", type: "keyword", detail: "throw" },
    { label: "anyar", type: "keyword", detail: "new" },
    { label: "paten", type: "keyword", detail: "const" },
    { label: "ono", type: "keyword", detail: "let" },
    { label: "sambil", type: "keyword", detail: "async" },
    { label: "enteni", type: "keyword", detail: "await" },
    { label: "nyoba", type: "keyword", detail: "try" },
    { label: "nompo", type: "keyword", detail: "catch" },
    { label: "akhire", type: "keyword", detail: "finally" },
    { label: "milih", type: "keyword", detail: "switch" },
    { label: "kasus", type: "keyword", detail: "case" },
    { label: "asale", type: "keyword", detail: "default" },
    { label: "mandheg", type: "keyword", detail: "break" },
    { label: "lanjutna", type: "keyword", detail: "continue" },
    { label: "warisan", type: "keyword", detail: "extends" },
    { label: "saka", type: "keyword", detail: "of" },
    { label: "ing", type: "keyword", detail: "in" }
  ];

  const atoms = [
    { label: "bener", type: "atom", detail: "true" },
    { label: "salah", type: "atom", detail: "false" },
    { label: "suwung", type: "atom", detail: "null" },
    { label: "rajelas", type: "atom", detail: "undefined" },
    { label: "iki", type: "atom", detail: "this" }
  ];

  const builtins = [
    { label: "tampilno", type: "function", detail: "console.log" },
    { label: "tampil.tabel", type: "function", detail: "console.table" },
    { label: "tampil.ngadat", type: "function", detail: "console.error" },
    { label: "tampil.awas", type: "function", detail: "console.warn" },
    { label: "tampil.kabar", type: "function", detail: "console.info" },
    { label: "jupuk", type: "function", detail: "querySelector" },
    { label: "jupukId", type: "function", detail: "getElementById" },
    { label: "jupukKabeh", type: "function", detail: "querySelectorAll" },
    { label: "kirim", type: "function", detail: "fetch" },
    { label: "dokumen", type: "variable", detail: "document" },
    { label: "layar", type: "variable", detail: "window" },
    { label: "konstruktor", type: "function", detail: "constructor" }
  ];

  const operators = [
    { label: "lan", type: "operator", detail: "&&" },
    { label: "utawa", type: "operator", detail: "||" },
    { label: "padhaKaro", type: "operator", detail: "===" },
    { label: "oraPadha", type: "operator", detail: "!==" },
    { label: "gedhePadha", type: "operator", detail: ">=" },
    { label: "cilikPadha", type: "operator", detail: "<=" },
    { label: "gedhe", type: "operator", detail: ">" },
    { label: "cilik", type: "operator", detail: "<" }
  ];

  const snippets = [
    { label: "yen-block", type: "keyword", detail: "if block", apply: "yen () {\n\t\n}" },
    { label: "yen-ora", type: "keyword", detail: "if-else block", apply: "yen () {\n\t\n} ora {\n\t\n}" },
    { label: "kanggo-loop", type: "keyword", detail: "for loop", apply: "kanggo (ono i = 0; i cilik 10; i++) {\n\t\n}" },
    { label: "nganti-loop", type: "keyword", detail: "while loop", apply: "nganti () {\n\t\n}" },
    { label: "fungsi-block", type: "keyword", detail: "function", apply: "fungsi () {\n\t\n}" },
    { label: "kelas-block", type: "keyword", detail: "class", apply: "kelas {\n\tkonstruktor() {\n\t\t\n\t}\n}" },
    { label: "nyoba-block", type: "keyword", detail: "try-catch", apply: "nyoba {\n\t\n} nompo (e) {\n\t\n}" }
  ];

  const all = [...keywords, ...atoms, ...builtins, ...operators, ...snippets];
  const lower = word.text.toLowerCase();
  const filtered = all.filter(o => o.label.toLowerCase().startsWith(lower));
...
function createJawaScriptLanguageSupport() {
  ...
  const jawaStream = {
    ...
    token(stream, state) {
      ...
      if (/[\p{L}_$]/u.test(ch)) {
        const match = stream.match(/^[\p{L}\p{N}_$]+/u);
        if (!match) { stream.next(); return null; }
        const word = match[0];
  ...
  const extensions = [new LanguageSupport(jawaLanguage)];

  // Add autocomplete if available
  try {
    const ac = cm.autocomplete;
    if (ac && typeof ac.autocompletion === "function") {
      extensions.push(ac.autocompletion({ override: [jawaCompletionSource] }));
    }
  } catch (e) {
    console.warn("[JawaScript] Autocomplete not available:", e);
  }
...
function init(baseUrl, $page) {
  ...
  commandsRef.addCommand({
    name: EXPORT_COMMAND,
    description: "JawaScript: Export compiled JavaScript to .js file",
    exec: handleExport
  });
...
function unmount() {
  ...
  commandsRef?.removeCommand(EXPORT_COMMAND);
